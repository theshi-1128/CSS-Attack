...
3.3 Standalone 模式 Standalone 模式可以独立于图存储运行，直接从文本文件或 ODPS 读取 Edgelist 形式的图数据。在 TuGraph/build 目录下执行 make bfs_standalone 即可得到 bfs_standalone 文件,该文件生成与 TuGraph/build/output/algo 文件夹下。运行：在 TuGraph/build 目录下执行./output/algo/bfs_standalone -–type [type] –-input_dir [input_dir] -–vertices [vertices] --root [root] –-output_dir [output_dir]
...
描述
CPython
C语言实现Python及其解释器（JIT编译器）
ctypes
Python标准库
SWIG
开发工具，封装native程序接口
Pybind11/Boost.python/Nanobind
轻量级头文件库
Cython
Python的超集，使用C语言特性，静态编译
